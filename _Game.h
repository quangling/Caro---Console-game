...
public:
    ...
    void Undo(int pX, int pY);
    void Redo(int pX, int pY, int check);

public:
    void startGame(); // Khởi tạo game . Bắt đầu game
    void exitGame(); // Thoát Game
    void SaveGame(int n, int scoreP1, int scoreP2, int scoreP3);  // Lưu Game đang chơi với biến n là chế độ và lượt và điểm win, lose, draw.
...
public:
    ...
    // Đảo lượt chơi
    void TimKiemNuocDi2(); // khó
    void TimKiemNuocDi1(); // dê
    void TimKiemNuocDiDe();
    void TimKiemNuocDi3();
